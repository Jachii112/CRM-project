...
class purchaseOrderController extends Controller
{
    ...
    public function completeOrder($orderNo)
    {
        ...
        try{
            ...
            return redirect()->route('materials.inventory')->with('success', 'Purchase success');
        } catch (\Exception $e) {
            // Handle exceptions and log the error
            error_log('Error: ' . $e->getMessage());

            // Return an error response or redirect with an error message
            return Redirect()->route('materials.inventory')->with('error', 'Error completing the order');
        }
