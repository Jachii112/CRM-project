...
class purchaseOrderController extends Controller
{
    ...
    public function completeOrder($orderNo)
    {
        ...
        if ($order) {
            // Check if the ordered_date is not null, which implies the order is completed
            if ($order->ordered_date === null) {
                // If the order is not completed, update the ordered_date to mark it as completed
                $order->ordered_date = Carbon::now(); // Update with your timestamp column name

                // Save the changes
                $order->save();

                // Redirect to the materials list or any other page as needed
                return Redirect::route('materials.inventory')->with('success', 'Order completed successfully');
            } else {
                // Handle the case where the order is already completed
